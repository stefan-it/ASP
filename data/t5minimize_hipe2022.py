...
def minimize_partition(
    ...
    input_path = f"{input_dir}/hipe2022_{name}.json"

    output_path = f"{output_dir}/{name}.jsonlines"
...
def get_subtokens(word):
    word = normalize_word(word, "german")
    if word == "(" or word == "[":
...
def minimize_language(entity_labels, stats, tokenizer, input_dir, output_dir
):
    ...
    for name in ["dev", "test", "train"]:
        minimize_partition(name, entity_labels, stats, tokenizer, input_dir, output_dir)
    return

if __name__ == "__main__":
    input_dir = sys.argv[1]
    output_dir = sys.argv[2]
    hf_model_name = sys.argv[3]

    tokenizer = T5Tokenizer.from_pretrained(hf_model_name, model_max_length=4096)

    typefile = f"{input_dir}/hipe2022_types.json"
    ...
    minimize_language(entity_labels, stats, tokenizer, input_dir, output_dir)
